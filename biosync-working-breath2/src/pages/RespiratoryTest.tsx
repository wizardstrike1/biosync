...
// keep near top, before component usage
...
const RespiratoryTest = () => {
  ...
  const [spectrumData, setSpectrumData] = useState<number[]>(
    () => new Array(SPECTRUM_BAR_COUNT).fill(IDLE_BAR_HEIGHT),
  );
  ...
  const resetSpectrum = () => {
    setSpectrumData(Array.from({ length: SPECTRUM_BAR_COUNT }, () => IDLE_BAR_HEIGHT));
  };

  const startSpectrumLoop = () => {
    const tick = () => {
      const analyser = analyserNodeRef.current;
      const freqData = spectrumDataRef.current;
      ...
      setSpectrumData(aggregateSpectrumBars(freqData, SPECTRUM_BAR_COUNT));
      ...
      const finishCapture = async () => {
        try {
          ...
          try {
            ...
            const payload = (await response.json()) as Partial<LungAnalysisResponse> & { error?: string };

            if (!response.ok) {
              throw new Error(payload.error ?? "Server lung analysis failed.");
            }

            if (payload.source !== "lung-cnn-pth" || typeof payload.healthPercent !== "number") {
              throw new Error("Model response missing or invalid.");
            }

            result = payload as LungAnalysisResponse;
          } catch (e) {
